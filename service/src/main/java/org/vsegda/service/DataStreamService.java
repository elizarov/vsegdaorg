...
public class DataStreamService {
    ...
    public static List<DataStream> getDataStreams() {
        return DataStreamStorage.INSTANCE.queryDataStreams();
    }
    ...
    public static DataStream resolveDataStreamByTag(String tag, boolean createIfAbsent) {
        DataStream stream = DataStreamStorage.INSTANCE.loadDataStreamByTag(tag);
        if (stream == null && createIfAbsent) {
            log.info("Creating new data stream with tag=" + tag);
            // find last stream
            stream = DataStreamStorage.INSTANCE.loadLastDataStream();
    ...
    public static DataStream resolveDataStreamById(long id, boolean createIfAbsent) {
        DataStream stream = DataStreamStorage.INSTANCE.loadDataStreamById(id);
        if (stream == null && createIfAbsent) {
    ...
    public static void removeDataStream(DataStream stream) {
        DataStreamStorage.INSTANCE.deleteDataStream(stream);
    }
}
