...
public class DataRequest extends AbstractRequest {
    ...
    private TimePeriod span = DataItemService.INSTANCE.getDEFAULT_SPAN();
    ...
    public List<DataItem> queryList() {
        ...
        if (id != null) {
            ...
            result.sort(Collections.reverseOrder(DataItem.Companion.getORDER_BY_TIME()));
        }
        return result;
    }
    ...
    public Map<DataStream, List<DataItem>> queryMap() {
        ...
        Map<DataStream, List<DataItem>> map = new LinkedHashMap<>();
        if (id == null) {
            for (DataStream stream : DataStreamService.getDataStreams())
                map.put(stream, Collections.singletonList(DataItemService.INSTANCE.getLastDataItem(stream)));
        } else {
            for (String code : this.id) {
                ...
                List<DataItem> items = new ArrayList<>(DataItemService.INSTANCE.getDataItems(stream, from, to, n));
                filter(items);
